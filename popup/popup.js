...
// MD5实现 (RFC 1321, 公有领域) - 用于CompleteMultipartUpload的Content-MD5头
function md5Base64(str) {
  function toUtf8(s) { return unescape(encodeURIComponent(s)); }
  function cmn(q,a,b,x,s,t){a=(((a+q)+(x))+t)>>>0;a=((a<<s)|(a>>>(32-s)))>>>0;return(a+b)>>>0;}
  ...
  const s=toUtf8(str);const st=md51(s);
  const bytes=new Uint8Array(16);
  for(let i=0;i<4;i++)for(let j=0;j<4;j++)bytes[i*4+j]=(st[i]>>>(j*8))&0xFF;
  let bin='';for(let i=0;i<16;i++)bin+=String.fromCharCode(bytes[i]);
  return btoa(bin);
}
...
async function buildOSSAuth(creds, method, contentType, date, canonicalizedResource, contentMd5 = '') {
